...
class VAEXperiment(pl.LightningModule):
  ...
  def training_step(self, batch, batch_idx, optimizer_idx = 0):
    ...
    self.log_dict({key: val.item() for key, val in train_loss.items()}, on_epoch=True, on_step=False, sync_dist=True)
  ...
  def validation_step(self, batch, batch_idx, optimizer_idx = 0):
    ...
    self.log_dict({f"val_{key}": val.item() for key, val in val_loss.items()}, on_epoch=True, on_step=False, sync_dist=True)
